...
- **Every marked window vanishes onto `special:cloak`.** That move is the only protection claim. Hyprland does not expose dwindle/master tree JSON, so a tiled window cannot be restored losslessly after leaving the layout. If any marked window is tiled, cloak is **refused** (toast) rather than claiming protection with `alpha 0`/`nofocus` on a still-present tile. Verify: each marked address is on `special:cloak` in `hyprctl -j clients` before the plugin reports `cloaked`. `tests/live-roundtrip.sh` round-trips floating vanish on a live compositor; GitHub CI does not start Hyprland and does not claim to.
