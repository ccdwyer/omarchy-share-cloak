...
- **Floating marked windows vanish onto `special:cloak`.** **Tiled marked windows stay put** and are hidden with `hl.dsp.window.set_prop` `no_screen_share=1`, `opacity=0`, `no_focus=1` (Hyprland 0.56 dynamic effects). Classic `hyprctl setprop alpha` is gone on 0.56. Verify tiled hides with `hyprctl getprop address:… no_screen_share`. Uncloak reverses those props. Cover cards use the window's live geometry. `tests/live-roundtrip.sh` round-trips floating vanish; GitHub CI does not start Hyprland.
...
- Writing Hyprland config only after an explicit bar **Set hotkey** click (or `installBinds` IPC). First load only *scans* `hyprctl -j binds` and never writes. Occupied combos (including stock Omarchy hotkeys) are skipped or replaced with Super+Alt variants. Never `hl.unbind` of someone else's shortcut. Notify with `omarchy notification send` only after a successful write. The bar shows current hotkeys when set; right-click / `removeBinds` strips the marked `-- BEGIN io.github.chris.share-cloak` block from `~/.config/hypr/bindings.lua`. Previously owned runtime Super+F9/F10 keyword binds (from an older auto-install) are still torn down on disable via `Quickshell.execDetached` of `compat/unbind-owned.sh`, which re-reads `hyprctl -j binds` and unbinds only exact owned exec commands (plugin id + expected method + empty arg). The helper outlives `Component.onDestruction`. Python3 missing or hyprctl failure → no unbind.
